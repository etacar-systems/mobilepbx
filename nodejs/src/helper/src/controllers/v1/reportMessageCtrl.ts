...
const getAllRecord = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const total_messageCount = await message_report.find(find_conditon).countDocuments()
        const reportMessageData = await message_report.find(find_conditon).populate({
            path: "reporter_id",
            model: "user",
...
const ReportMessgeRemove = async (req: Request, res: Response, next: NextFunction) => {
    try {
        var report_id = req.params.report_id
        if (report_id) {
