...
const deleteRecording = async (req: Request, res: Response) => {
        try {
                ...
                const deleteData = await system_recording.findByIdAndUpdate({
                        _id: record_id
                }, {
                        is_deleted: 1
                }, {
...
const getrecodlist = async (req: Request, res: Response, next: NextFunction) => {
        try {
                ...
                const recording_list: any = await system_recording.find(find_query)
                        .sort({ createdAt: -1 })
                        .limit(limit)
                        .skip(skip);

                const recording_total_counts: any = await system_recording.find(find_query)
                        .countDocuments();
