...
const getAllRecord = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const total_reportData = await user_report.find(find_conditon).countDocuments()
    
        const reportData = await user_report.find(find_conditon).populate({
            path: "cid",
            model: "company",
            select: "company_name"
        }).populate({
        ...
        const report_data_tmp = await Promise.all(reportData.map(async item => {
            let cloneItem: any = item.toObject()
            let report_id_tmp = null
            let group_admins: any[] = []
            if (cloneItem.isGroup == 1) {
                report_id_tmp = await group.findById({
                ...
                group_admins = await group_members.find({
                ...
                report_id_tmp = await user.findById({
                    _id: cloneItem.report_id
                }).select("first_name last_name user_extension")
            }
...
const updateReportUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
