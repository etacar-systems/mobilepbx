...
const getDetailsById = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.params._id) {
    let groupsDetails: any = await group.findOne({
      cid: cid,
      _id: req.params._id,
      is_deleted: 0,
    }).populate({
    ...
    }).select("description group_image is_deleted created_by createdAt group_name is_admin_send_message public_link public_link_expire_time")

    let get_group_users_detail = await group_members.find({
    ...
    let get_group_notification: any = await notification_setting_users.findOne({
...
const getGroupUserRole = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
const getGroupUsers = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (group_id) {
    ...
    const temp_group_users = await group_members.find({
      group_id: group_id
    }).populate({
      path: "member_id",
      model: "user",
