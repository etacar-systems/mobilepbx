...
const createBroadcast = async (
  ...
  try {
    ...
    let get_broadcast_list = await broadcast_users
      .findById({
...
const getBroadCastList = async (
  ...
  try {
    ...
    let get_broadcast = await broadcast_users
      .find({
        uid: uid,
      })
      .populate({
...
const deleteBroadcast = async (
  ...
  try {
    ...
    var uid = user_detail?.uid;
    if (_id) {
      const post = await broadcast_users
        .findOneAndDelete(
          {
            _id: _id,
            uid: uid,
...
const getBroadConveration = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
const getBroadcastUsers = async (
  ...
  try {
    ...
    let get_broadast_users = await broadcast_users
      .findOne({
        _id: broadcast_id,
      })
      .populate({
