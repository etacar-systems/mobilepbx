...
const UpdateUserMessageSetting = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (message_disappear_type == 0) {
      message_dissapear_time = moment(current_time)
        .add(1, "day")
    ...
    if (isgroup) {
      if (message_disappear_type !== undefined) {
        ...
        if (get_user_data !== null) {
          ...
          user_message_disappear.message_disappear_type =
            message_disappear_type;
          user_message_disappear.message_dissapear_time =
...
const getUserMessageSetting = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
    ...
    if (uid && cid && isgroup !== undefined) {
      if (isgroup) {
        ...
        if (find_user_message_detail) {
          message_disappear_type =
            find_user_message_detail.message_disappear_type;
        }
      } else {
        let find_user_message_detail: any = await message_setting_users.findOne(
          {
