...
export const connect = async (server: any) => {
  ...
  io.on("connection", async (socket: any) => {
    ...
    console.log(
      "socket.handshake.query.cid",
      socket.handshake.query.cid != "undefined"
    );
    if (
      socket.handshake.query.cid != null &&
      socket.handshake.query.cid != undefined &&
      socket.handshake.query.uid != null &&
      socket.handshake.query.uid != undefined &&
      socket.handshake.query.cid &&
      socket.handshake.query.uid &&
      socket.handshake.query.uid.length !== 0 &&
      socket.handshake.query.cid.length !== 0 &&
      socket.handshake.query.device_id != null &&
      socket.handshake.query.last_message_time != null &&
      socket.handshake.query.device_id != undefined &&
      socket.handshake.query.last_message_time != undefined &&
      socket.handshake.query.cid != "undefined" &&
      socket.handshake.query.uid != "undefined"
    ) {
      console.log("socket.handshake.query.uid", socket.handshake.query.uid);
      ...
      let create_uid: any = new mongoose.Types.ObjectId(
        socket.handshake.query.uid
      );
      ...
      let create_uid_tmp: any = new mongoose.Types.ObjectId(
        socket.handshake.query.uid
      );
      ...
      let get_block_user_detail = await user_block.aggregate([
        {
          $match: {
            $or: [{ block_by: create_uid_tmp }, { block_id: create_uid_tmp }],
          },
        },
        {
          $project: {
      ...
      let block_userids: any[] =
        get_block_user_detail.length > 0
          ? get_block_user_detail[0].blockedUsers
          : [];
      ...
      await Promise.all(
        socket_users.map(async (item) => {
          if (
            ...
            !block_userids.includes(item.uid.toString())
          ) {
      ...
      if (tmp_date) {
        ...
        await Promise.all(
          socket_users.map(async (item) => {
            if (
              item.uid.toString() === create_uid.toString() &&
              item.cid.toString() === socket.handshake.query.cid.toString() &&
              item.device_id.toString() ===
                socket.handshake.query.device_id.toString()
            ) {
          ...
          await Promise.all(
            getSenderList.map(async (item: any) => {
              ...
              socket_users.map(async (row: any) => {
                if (
                  !item.delete_message_users.includes(row.uid.toString()) &&
                  ...
                  let sender_detail: any = {
                    name:
                      get_user_detail.first_name +
                      " " +
                      get_user_detail.last_name,
            ...
            get_group_mes_detail.map(async (item: any) => {
              ...
              socket_users.map(async (row: any) => {
                if (
                  ...
                  let post: any = item.toObject();
                  let get_sender_detail: any = await user
                    .findById({
                      _id: item.sender_id,
                      is_deleted: 0,
                    })
        ...
        try {
          ...
          if (
            ...
            if (get_user_detail_sender) {
              originalName_sender =
                get_user_detail_sender.first_name +
                " " +
                get_user_detail_sender.last_name;
            }

            let check_user_delted_chat_sender: any =
            ...
            let check_user_delted_chat_reciver: any =
              await user.findOneAndUpdate(
                {
                  _id: sender_id,
            ...
            post = post.toObject();
            post.tmp_message_id = data.tmp_message_id
              ? data.tmp_message_id
              : null;
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              if (
                item.uid.toString() === sender_id.toString() &&
              ...
              if (
                item.uid.toString() !== sender_id.toString() &&
                !left_group_member.includes(item.uid) &&
                reciver_ids_arr.includes(item.uid) &&
                item.cid.toString() === cid.toString()
              ) {
          ...
          if (isgroup && data.group_id !== undefined) {
            ...
            await Promise.all(
              reciver_ids_arr.map(async (row: any) => {
          ...
          if (isgroup == 0 && sender_id !== undefined) {
            ...
            notification_type =
              MESSAGE.PUSH_NOTIFICATION_TYPE.ONETOONE.toString();
            let get_user_detail: any = await user.findById({
              _id: sender_id,
            });
            if (get_user_detail) {
        ...
        try {
          if (data) {
            ...
            if (isgroup !== undefined && isgroup && group_id !== null) {
              ...
              let get_message_all_count: any = await group_message_status
                .find({
                  message_id: message_id,
                })
                .countDocuments();

              let get_message_deliver_count: any = await group_message_status
                .find({
                  message_id: message_id,
                  delivery_type: 2,
                })
              ...
              let get_message_read_count: any = await group_message_status
                .find({
                  message_id: message_id,
                  delivery_type: 3,
                })
              ...
              let get_message_delivery_type: any =
                await group_conversation.findById({
                  _id: message_id,
                });
            ...
            await Promise.all(
              socket_users.map(async (item) => {
                if (
                  item.uid.toString() === sender.toString() &&
                  item.cid.toString() === cid.toString()
                ) {
                  sendrSocketIds.push(item.socket_id);
                }
        ...
        if (data) {
          ...
          if (
            ...
            await Promise.all(
              socket_users.map(async (item) => {
                if (
                  item.uid.toString() === block_by.toString() &&
                  item.cid.toString() === cid.toString()
                ) {
                  mySocketIds.push(item.socket_id);
                }
                if (
                  item.uid.toString() === block_id.toString() &&
                  item.cid.toString() === cid.toString()
                ) {
                  reciverSocketIDs.push(item.socket_id);
                }
        ...
        if (data) {
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              if (
                data.cid === item.cid &&
                item.uid.toString() === reciver_id.toString()
              ) {
                mySocketIds.push(item.socket_id);
              }
            })
      ...
      socket.on("send_create_group", async function (data: any) {
        ...
        let new_message: String =
          get_user_nm.first_name +
          " " +
          get_user_nm.last_name +
          " Created a Group";
        ...
        post_convarsation.schedule_time = data.schedule_time
          ? data.schedule_time
          : null;
        post_convarsation.block_message_users = [];
        await post_convarsation.save();

        let get_group_user_tmp: any = await group_members
          .find({
            group_id: temPost._id,
          })
          .populate({
      ...
      socket.on("add_group_members", async function (data: any) {
        ...
        if (
          ...
          if (get_group_detail) {
            ...
            let check_already_add_users: any[] = await group_members
              .aggregate([
                {
                  $match: {
            ...
            check_already_add_users =
              check_already_add_users.length > 0
                ? check_already_add_users[0].member_ids
                : [];
            check_already_add_users = check_already_add_users.map((id) =>
              id.toString()
            );
            ...
            get_group_users_arr = get_group_users_arr.concat(
              check_already_add_users
            );
            get_group_users_arr = get_group_users_arr.concat(member_ids);
            get_group_users_arr = get_group_users_arr.filter(
              (rows: any, index: any, arr: any) => arr.indexOf(rows) === index
            );

            add_members_array = member_ids.filter(
              (item) => !check_already_add_users.includes(item)
            );
          }
          console.log("get_group_users_arr", get_group_users_arr);
          console.log("add_members_array", add_members_array);
          let message: any =
            get_user_nm?.first_name + " " + get_user_nm?.last_name + " added ";
          if (get_group_detail) {
            ...
            if (add_members_array.length > 0) {
              await Promise.all(
                add_members_array.map(async (items: any) => {
            ...
            await Promise.all(
              member_ids.map(async (items: any) => {
                ...
                let tmp_msg =
                  message +
                  get_reciver_nm?.first_name +
                  " " +
                  get_reciver_nm?.last_name;
            ...
            group_add_message = await group_conversation.insertMany(
              group_message
            );
            let added_member_detail: any = await group_members.insertMany(
              add_member_details
            );
            ...
            await Promise.all(
              socket_users.map(async (item) => {
                if (
                  group_detail_tmp.group_users.includes(item.uid.toString()) &&
                  !member_ids.includes(item.uid.toString())
                ) {
                  mySocketIds.push(item.socket_id);
                }
            ...
            let get_group_users: any = await group_members
              .find({
                group_id: group_id,
              })
              .populate({
      ...
      socket.on("remove_group_members", async function (data: any) {
        ...
        let message: any =
          get_user_nm?.first_name + " " + get_user_nm?.last_name + " Removed ";
        if (get_group_detail && member_id) {
          ...
          groupPost.media_type =
            MESSAGE.MESSAGE_MEDIA_TYPES.GROUP_MEMBER_REMOVE;
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              if (
                group_detail_tmp.group_users.includes(item.uid.toString()) &&
                member_id.toString() != item.uid.toString()
              ) {
                mySocketIds.push(item.socket_id);
              }
          ...
          if (
            ...
            if (status) {
              new_message =
                ...
                " Make Admin" +
                " " +
                get_addedmember_deatil.first_name +
                " " +
                get_addedmember_deatil.last_name;
            } else {
              new_message =
                get_user_nm.first_name +
                " " +
            ...
            post_convarsation.message = new_message;
            post_convarsation.message_type = 0;
            post_convarsation.media_type = status
              ? MESSAGE.MESSAGE_MEDIA_TYPES.GROUP_MAKE_ADMIN
              : MESSAGE.MESSAGE_MEDIA_TYPES.GROUP_REMOVE_FROM_ADMIN;
            post_convarsation.reply_message_id = "";
            post_convarsation.schedule_time = data.schedule_time
              ? data.schedule_time
              : null;
            ...
            await Promise.all(
              socket_users.map(async (item) => {
                if (
                  member_id !== item.uid.toString() &&
                  groupUsersData.includes(item.uid.toString())
                ) {
                  mySocketIds.push(item.socket_id);
                }
      ...
      socket.on("forward_message", async function (data: any) {
        ...
        let message_details_arr: any[] =
          get_messages_detail.concat(get_group_messages);
        let inserted_messages: any[] = [];
        await Promise.all(
          reciver_ids.map(async (item: any) => {
            if (item.isgroup == 0) {
              let insert_message_arr: any[] = [];
              await Promise.all(
                message_details_arr.map(async (row: any) => {
                  let get_check_block_by_reciver: any =
                    await user_block.findOne({
              ...
              let post: any = await group_conversation.insertMany(
                insert_message_arr
              );
              inserted_messages = inserted_messages.concat(post);
            }
        ...
        let one_to_oneMessages: any[] = inserted_messages.filter(
          (item) => item.receiver_id
        );
        let GroupMessages: any[] = inserted_messages.filter(
          (item) => item.group_id
        );
        const myACKSocketIds: any[] = [];
        await Promise.all(
          ...
          one_to_oneMessages.map(async (item: any) => {
            ...
            socket_users.map(async (row: any) => {
              if (
                !item.delete_message_users.includes(row.uid.toString()) &&
                row.uid.toString() === item.receiver_id.toString() &&
                row.uid.toString() != sender_id &&
                row.cid.toString() === cid.toString()
              ) {
                let post: any = item.toObject();
        ...
        await Promise.all(
          GroupMessages.map(async (item: any) => {
            ...
            if (item.group_id !== undefined) {
              notification_type =
                MESSAGE.PUSH_NOTIFICATION_TYPE.GROUP.toString();
      ...
      socket.on("leave_goup", async function (data: any) {
        ...
        await Promise.all(
          socket_users.map(async (item) => {
            ...
            if (
              item.uid.toString() !== uid.toString() &&
              group_members_list.includes(item.uid) &&
              item.cid.toString() === cid.toString()
            ) {
              ReciverSocketIds.push(item.socket_id);
            }
      ...
      socket.on("mute_notification", async function (data: any) {
        if (data) {
          ...
          let unmute_date: any = "";
          if (
            notification_mute_type !== undefined &&
            notification_mute_type == 1
          ) {
            let get_current_date = moment()
              .utc()
              .format("YYYY-MM-DDTHH:mm:ss.sssZ");
            let get_current_date_seconds = moment(get_current_date)
              .utc()
              .set({ second: 0, millisecond: 0 });
            unmute_date = moment(get_current_date_seconds)
              .utc()
              .add(8, "hours")
              .format("YYYY-MM-DDTHH:mm:ss.sssZ");
          } else if (
            notification_mute_type !== undefined &&
            notification_mute_type == 2
          ) {
            let get_current_date = moment()
              .utc()
              .format("YYYY-MM-DDTHH:mm:ss.sssZ");
          ...
          if (
            notification_mute_id !== undefined &&
            cid !== undefined &&
            uid !== undefined &&
            isgroup !== undefined
          ) {
            ...
            if (get_user_data !== null) {
              user_notification =
                await notification_setting_users.findOneAndUpdate(
                  {
                    cid: cid,
                    uid: uid,
                    isgroup: isgroup,
              ...
              if (get_mute_member_detail) {
                mute_member_name =
                  get_mute_member_detail?.first_name +
                  " " +
                  get_mute_member_detail?.last_name;
              }
            } else {
      ...
      socket.on("send_edit_group", async function (data: any) {
        ...
        if (
          ...
          cid !== undefined &&
          cid !== null &&
          is_admin_send_message !== undefined &&
          is_admin_send_message !== null
        ) {
        ...
        await Promise.all(
          socket_users.map(async (item) => {
            if (
              group_users_arr.includes(item.uid.toString()) &&
              item.cid.toString() === cid.toString()
            ) {
              mySocketIds.push(item.socket_id);
            }
      ...
      socket.on("user_messages", async function (data: any, callback: any) {
        if (data && data.last_message_date !== null) {
          ...
          const Message_date: any = moment(data.last_message_date)
            .utc()
            .format("YYYY-MM-DDTHH:mm:ss.sssZ");
          const getSenderList = await conversation.find({
            is_deleted: 0,
            $or: [{ receiver_id: data.uid }, { sender_id: data.uid }],
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              if (
                ...
                item.cid.toString() === data.cid.toString()
              ) {
                myACKSocketIds.push(item.socket_id);
              }
            })
      ...
      socket.on("update_group_message_setting", async function (data: any) {
        ...
        await Promise.all(
          socket_users.map(async (item) => {
            if (
              group_members.includes(item.uid) &&
              item.cid.toString() === cid.toString()
            ) {
              ReciverSocketIds.push(item.socket_id);
            }
        ...
        try {
          ...
          if (
            ...
            if (get_user_detail) {
              ...
              await Promise.all(
                socket_users.map(async (item) => {
                  if (item.uid == data.uid && item.cid == data.cid) {
                    mySocketIds.push(item.socket_id);
                  }
                })
              );
      ...
      socket.on("initalize_call", async function (data: any) {
        try {
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              if (reciver_id.includes(item.uid.toString())) {
                mySocketIds.push(item.socket_id);
              }
            })
          );
      ...
      socket.on("read_all_delivered_message", async function (data: any) {
        try {
          ...
          if (
            ...
            mongoose.Types.ObjectId.isValid(recevier_id)
          ) {
            let get_msgess_ids: any[] = await group_message_status
              .find({
                receiver_id: uid,
            ...
            let updated_group_receive_msg: any =
              await group_message_status.updateMany(
                {
                  receiver_id: uid,
                  group_id: recevier_id,
                  delivery_type: 2,
            ...
            let update_group_message_ids: any =
              await group_conversation.aggregate([
                {
                  $match: {
                        ...
                        branches: [
                          {
                            case: {
                              $eq: [
                                {
                                  $size: "$group_messages_total_deliver_count",
                                },
                                { $size: "$group_messages_total_count" },
                              ],
                            },
                            then: 2,
                          },
                          {
                            case: {
                              $eq: [
                                { $size: "$group_messages_total_read_count" },
                                { $size: "$group_messages_total_count" },
                              ],
                            },
                            then: 3,
            ...
            let updated_group_message_ids: any = await group_conversation
              .find({
                _id: { $in: update_group_message_ids },
              })
              .distinct("sender_id");

            message_sender_id_arr = updated_group_message_ids.map(
              (row: any) => {
                return row.toString();
              }
            );
          ...
          if (
            ...
            let get_read_message: any = await conversation
              .find({
                receiver_id: uid,
          ...
          await Promise.all(
            socket_users.map(async (item) => {
              ...
              if (
                item.uid.toString() !== uid.toString() &&
                message_sender_id_arr.includes(item.uid) &&
                item.cid.toString() === cid.toString()
              ) {
                let get_sender_message_tmp: any = sender_msges_arr.filter(
                  (row: any) => row.sender_id.toString() === item.uid.toString()
                );
                //console.log("get_sender_message_tmp",get_sender_message_tmp)
                let updated_message_id: any = get_sender_message_tmp.map(
                  (row: any) => {
                    return row._id;
                  }
                );
      ...
      socket.on("delivere_all_message", async function (data: any) {
        try {
          ...
          console.log(
            "update_group_message_ids aggreagte",
            update_group_message_ids
          );
          ...
          let updatedGroupMessageIds_sendeids =
            await group_conversation.aggregate([
              {
                $match: {
                  _id: { $in: get_msgess_ids },
          ...
          let tmp_meesage_ids_arrys: any[] =
            updatedGroupMessageIds_sendeids.length > 0
              ? updatedGroupMessageIds_sendeids[0].sender_ids
              : [];
          message_sender_id_arr = message_sender_id_arr.concat(
            tmp_meesage_ids_arrys
          );
          ...
          let get_read_message_obj: any = await conversation
            .find({
              receiver_id: uid,
              delivery_type: 1,
            })
          ...
          let tmp_meesage_ids_arrys_onetoone: any[] =
            get_deliver_msg_senders.length > 0
              ? get_deliver_msg_senders[0].sender_ids
              : [];
          message_sender_id_arr = message_sender_id_arr.concat(
            tmp_meesage_ids_arrys_onetoone
          );
      ...
      socket.on("register_extantions", async function (data: any) {
        try {
          ...
          let extensions_obj: any =
            JSON.parse(freeSwitch_client_response) || [];
          ...
          let get_role_details: any = await role
            .find({
              type: { $ne: 1 },
            })
            .distinct("_id");
      ...
      socket.on("unread_message_count", async function (data: any) {
        try {
          ...
          const get_unread_message_count = await user.aggregate([
                          ...
                          $filter: {
                            ...
                            cond: {
                              $and: [
                                ...
                                {
                                  $not: {
                                    $in: [uid, "$$msg.delete_message_users"],
                                  },
                                },
                              ],
                            },
            ...
            },
            {
              $project: {
                ...
                groupMessageCount: {
                  $arrayElemAt: ["$groupMessageCount.count", 0],
                },
              },
            },
            {
          ...
          let total_msg_count: any =
            get_unread_message_count[0].total_msg_count;
