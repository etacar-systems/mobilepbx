	...
	// END 
        ...
        if ($destination_enabled == "true" || $destination_enabled == "false") {
            ...
            if($create_range > 0){
                ...
                $destination_number_new = substr($destination_number, -10);
                // Date :30-10-2024 Added by Atul 
               // $len=strlen($destination_number);
               // $len= $len -1;
                ...
                for ($i=0; $i < $create_range; $i++) { 
                    ...
                    //$destination_number_new = $dest_prefix.$destination_number_new + 1;

                    str_replace($dest_prefix, '', $destination_number_new);

                    $incrementedNumber = str_pad((int)$destination_number_new + 1, strlen($destination_number_new), '0', STR_PAD_LEFT);
                    $destination_number_new = $incrementedNumber;
                    $destination_number_new_2 = $dest_prefix.$incrementedNumber;


                    $query = "SELECT * FROM public.v_destinations WHERE destination_number ='$destination_number_new_2' and domain_uuid='$domain' ";
                    $check_destination = pg_query($con, $query);

                    if (pg_num_rows($check_destination) > 0 ) {

                        array_push($duplicate_arr,$destination_number_new_2);
                        ...
                        $result = $destination->post($con, $type, $destination_number_new_2, $caller_id_name, $caller_id_number, $destination_condition, $destination_action, $domain, $user, $description, $destination_enabled, $uuidDestination, $id, $destination_prefix,'import',$trunk_id);
