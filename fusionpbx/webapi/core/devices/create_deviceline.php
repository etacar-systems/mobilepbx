<?php
    ...
    if ($json_response['msg'] == 'true') {
        ...
        $deviceline_enabled = $data->deviceline_enabled;

        $uuid = Uuid::uuid4();
        // Get the UUID as a string
        $uuidString = $uuid->toString();

        if ($deviceline_enabled == 'true' || $deviceline_enabled == 'false') {
            $device->post_deviceline($con, $uuidString, $device_id, $domain_name, $deviceline_enabled, $device_linenumber, $device_server_address, $device_line_label, $device_line_displayname, $device_line_userid, $device_line_user_auth, $device_line_user_password, $device_line_port, $device_line_transport, $device_line_register_expire, $device_line_shared_line, $id);
        } else {
            echo json_encode([
                "message" => "Device Line Enabled Value Must be Either True or False !!"
            ]);
        }
    } else {
