<?php
    ...
    if ($json_response['msg'] == 'true') {
        ...
        $user = $data->user;
        $extension_password = $data->extension_password;
        ...
        if(!isset($check_ext['app'])){
            ...
            $uuidExtension = $uuid->toString();

            $uuid = Uuid::uuid4();
            // Get the UUID as a string
            $uuidExtensionUser = $uuid->toString();

            if ($extension_enabled == "true" || $extension_enabled == "false") {
                $extensions->post($con, $extension, $user, $extension_password, $account_code, $outbound_caller_id_name, $outbound_caller_id_number, $effective_caller_id_name, $effective_caller_id_number, $emergency_caller_id_name, $emergency_caller_id_number, $max_registrations, $limit_max, $user_record, $domain, $context, $description, $extension_enabled, $uuidExtension, $uuidExtensionUser, $id);
            } else {
                echo json_encode([
